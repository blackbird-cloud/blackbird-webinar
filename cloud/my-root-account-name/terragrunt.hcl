locals {
  ...
  iam_role     = get_env("CI", "false") == "true" && local.is_mgmt_account == false ? local.iam_role : null
  ...
  iam_role     = "arn:aws:iam::${local.account_id}:role/${local.global.github_role_name}"
}
